...
function bindSlideToggle(trigger, boxBody, content, openClass) {
    ...
    button.element.addEventListener('click', () => {
        if (button.active === false) { //проверяем меню, закрыто ли оно
            button.active = true; //если меню закрыто, то открываем
...
function start() {
    gapi.client.init({
        'apiKey': 'AIzaSyCQ2Jv2TAlHvSa8K-W1xfy2JkbBNpiEuEg',
...
function bindModal(cards) {
    cards.forEach(item => {
        item.addEventListener('click', (e) => {
            // отменили переход по ссылке (поведение браузреа по умолчанию)
            e.preventDefault();
...
// закрываем окошко (тыкая вне белой области)
